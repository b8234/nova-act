...
for k in ("NOVA_ACT_API_KEY", "NOVA_API_KEY", "AMAZON_NOVA_API_KEY"):
    if os.getenv(k):
        print(f"INFO: Unsetting {k} to use IAM auth for NovaAct.")
...
def main(
    ...
    bucket = s3_bucket_name or os.getenv("AWS_BUCKET_NAME")
    if not bucket:
        raise ValueError("Missing bucket. Pass --s3_bucket_name or set AWS_BUCKET_NAME in your environment.")
    """
    ...
    try:
        ...
        print("NovaAct authentication failed.")
        print("This IAM role is not allowlisted. Please request access here:")
        print("https://amazonexteu.qualtrics.com/jfe/form/SV_9siTXCFdKHpdwCa")
        raise
