...
def add_commute_distance(apartment: Apartment, caltrain_city: str, headless: bool) -> CaltrainCommute | None:
    with NovaAct(
        ...
        try:
            nova.act(
                f"Search for {caltrain_city} Caltrain station in the Bing Maps search bar and press enter. "
                "Do not click any external website links (like caltrain.com). "
                "In the left-hand place card for the station, click the 'Directions' button. "
                f"Enter '{apartment.address}' into the starting point field and press enter. "
                "Click the walking icon for directions."
            )
            result = nova.act("Return the shortest walking time and distance as JSON with hours, minutes, and miles.", schema=CaltrainCommute.model_json_schema())
